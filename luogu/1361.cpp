...
MF Max_Flow;
int a[192608],b[192608];
int main(){
#ifdef cnyali_lk
	freopen("1361.in","r",stdin);
	freopen("1361.out","w",stdout);
#endif
	int n,m,s,t,ans=0;
	n=read();
	...
	t=n+m+m+1;
	Max_Flow.n=t;
	for(int i=1;i<=n;++i)Max_Flow.add(s,i,a[i]),Max_Flow.add(i,t,b[i]);
	int cn=n,k,x;
	for(int i=1;i<=m;++i){
		k=read();
		int h=read();
		...
		for(int i=1;i<=k;++i){
			x=read();
			Max_Flow.add(cn-1,x,0x3f3f3f3f);
			Max_Flow.add(x,cn,0x3f3f3f3f);
		}
	}
