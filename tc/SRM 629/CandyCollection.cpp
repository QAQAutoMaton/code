...
// CUT begin
ifstream data("CandyCollection.sample");
...
bool do_test(vector<int> Type1, vector<int> Number1, vector<int> Type2, vector<int> Number2, int __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(Type1, Number1, Type2, Number2, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1519127128;
