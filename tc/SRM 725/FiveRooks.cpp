...
class FiveRooks {
    ...
    vector<int> find(vector<string> board) {
		for(int i=0;i<255;++i){
			...
			if(s==5){
				++t;
				for(int j=0;j<8;++j)if(i&(1<<j))ok[t][++fa]=j;
			}
		}
		for(int i=1;i<=t;++i)for(int j=1;j<=t;++j){
			for(int k=1;k<=5;++k){
				u[k]=ok[i][k];v[k]=ok[j][k];
			}	
			do{
				int ye=1;
				for(int k=1;ye&&k<=5;++k)if(board[u[k]][v[k]]=='.')ye=0;
				if(ye){

//					for(int k=1;ye&&k<=5;++k)putchar(board[u[k]][v[k]]);
					vector<int> s;	
					for(int k=1;k<=5;++k){
						s.push_back(u[k]);
						s.push_back(v[k]);
        ...
        return vector<int>{1,9,2,6,0,8,1,7};
    }
};
